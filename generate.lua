...
opt = {
  ...
  model = '/projectnb/dlearn/jcliff/videogan/checkpoints/ALL_UCF/iter33810_net.t7',
  batchSize = 128,
  gpu = 1,
  cudnn = 1,
}
...
paths.mkdir('allvis/')
WriteGif('allvis/gen.gif', gen) 
WriteGif('allvis/video.gif', video) 
WriteGif('allvis/videomask.gif', torch.cmul(video, mask))
WriteGif('allvis/mask.gif', mask)
image.save('allvis/static.jpg', image.toDisplayTensor(static))
